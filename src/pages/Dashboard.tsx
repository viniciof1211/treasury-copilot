...
export function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [cxpR, flujoR, projR, ingestR] = await Promise.all([
        ...
        querySQL(`SELECT compania, cuota, principal, intereses, vencimiento, banco, tipo, operacion, created_at, ingest_run_id FROM silver_finance.flujo_semanal ORDER BY vencimiento`),
  ...
  // ── Historical monthly trends (from ingest timestamps) ─────────────────────
  ...
  // ── Projection with break-even line ────────────────────────────────────────
  const projectionChart = useMemo(() => {
    // Build combined: historical months + projection months
    ...
    projection.forEach(p => {
      ...
      all.push({
        ...
        ingresos: Number(p.projected_inflows),
        egresos: Number(p.projected_outflows),
  ...
  flujo.forEach(r => {
    ...
    if (!flujoByBU[bu]) flujoByBU[bu] = { ingresos: 0, egresos: 0 };
    flujoByBU[bu].ingresos += Number(r.cuota) || 0;
  });
  cxp.forEach(r => {
    const bu = r.empresa || 'Sin BU';
    if (!flujoByBU[bu]) flujoByBU[bu] = { ingresos: 0, egresos: 0 };
    flujoByBU[bu].egresos += Number(r.monto_usd) || 0;
  });
  ...
  if (totalIntereses > 0) insights.push({ type: 'insight', text: `Carga financiera: ₡${(totalIntereses / 1000).toFixed(0)}K en intereses. Principal: ₡${(totalPrincipal / 1000).toFixed(0)}K.` });
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">
              Visión consolidada de cashflow, CxP y proyecciones en colones (₡) &middot; ARA Group
            </p>
          </div>
              ...
              {(Object.keys(PERIOD_LABELS) as TimePeriod[]).map(p => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${
                    period === p
                      ? 'bg-[#1A4A28] text-white shadow-sm'
        ...
        {loading && !hasData ? (
          ...
          <>
            ...
            {/* ─── 2. Cashflow Histórico + Proyección con Punto de Equilibrio ─── */}
            <Card>
              <CardHeader>
                ...
                {projectionChart.length > 0 ? (
                  <ResponsiveContainer width="100%" height={340}>
                    <ComposedChart data={projectionChart}>
                      ...
                      <ReferenceLine y={0} stroke={ARA_COLORS.red} strokeDasharray="4 4" label={{ value: 'Punto de Equilibrio ₡0', fill: ARA_COLORS.red, fontSize: 10 }} />
            ...
            {/* ─── 4. Top Proveedores + Clasificación CxP ─────────── */}
            ...
            {/* ─── 6. Historial de Ingestas (Data Persistence) ────── */}
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base">
